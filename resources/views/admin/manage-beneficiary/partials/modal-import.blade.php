        ...
        <div class="relative bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-4xl overflow-hidden font-sans">
            <div class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-[14px] font-bold uppercase tracking-widest text-slate-700">Import Massal Penerima Manfaat</h3>
                </div>
            ...
            <div class="p-8">
                ...
                <div id="import-step-1" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between p-4 bg-indigo-50 border border-indigo-100 rounded-xl">
                        ...
                        <a href="{{ route('admin.manage-beneficiary.template') }}" class="px-4 py-2 bg-indigo-600 text-white text-[10px] font-bold rounded-lg uppercase shadow-md hover:bg-indigo-700 transition-all">Unduh Template</a>
                    </div>
                ...
                <div id="import-step-2" class="hidden space-y-6">
                        ...
                        <div class="overflow-x-auto border border-slate-100 rounded-lg max-h-[300px]">
                            <table class="w-full text-[12px] text-left border-collapse">
                                <thead class="bg-slate-50 sticky top-0 shadow-sm">
                                    <tr>
                                        <th class="p-3 border-b font-bold text-slate-500 uppercase">Tipe Kelompok</th>
                    ...
                    <form action="{{ route('admin.manage-beneficiary.import') }}" method="POST" id="final-import-form">
                        @csrf
                        <div class="space-y-3 mb-6" id="import_db_options_container">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <label class="p-4 border rounded-xl cursor-pointer hover:bg-slate-50 flex flex-col gap-1 transition-all border-slate-200 has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50/30">
                                    ...
                                    <span class="text-[9px] text-slate-500 italic">Hanya menambah data baru. Data dengan ID/Kode PM yang sudah ada akan dilewati.</span>
                                </label>
                                ...
                                <label class="p-4 border rounded-xl cursor-pointer hover:bg-rose-50 flex flex-col gap-1 transition-all border-slate-200 has-[:checked]:border-rose-600 has-[:checked]:bg-rose-50/50">
                                    ...
                                    <span class="text-[9px] text-rose-400 italic font-medium">Hapus & Ganti semua data Penerima Manfaat.</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div id="import_user_progress" class="hidden py-6 flex flex-col justify-center items-center space-y-4">
                            ...
                            <p id="import_progress_desc" class="text-[10px] text-slate-400 font-medium text-center italic mt-2">Menyimpan data PM ke dalam sistem. Mohon bersabar.</p>
...
<script>
        ...
        async function renderPreview(data) {
            ...
            // Tampilkan Step 2 dan Status Loading
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                const code = row['KODE PM'] ? row['KODE PM'].toString() : '-';
                const name = row['NAMA PENERIMA MANFAAT'] ? row['NAMA PENERIMA MANFAAT'].substring(0, 25) + (row['NAMA PENERIMA MANFAAT'].length > 25 ? '...' : '') : '<span class="text-rose-500 italic">Kosong</span>';
                ...
                if (missingFields.length > 0) {
                    errors.push('Terdapat ' + missingFields.length + ' kolom wajib kosong');
                }

                // Validasi Tipe Kelompok
                const allowedGroup = ['sekolah', 'posyandu'];
                const validCategories = {
                    'sekolah': ['PAUD', 'TK', 'SD', 'SMP', 'SMA', 'SMK', 'Pesantren', 'RA', 'MI', 'MA', 'Pratama Widyalaya', 'Madyama Widyalaya', 'Utama Widyalaya', 'Utama Widyalaya Kejuruan'],
                    'posyandu': ['Ibu Hamil', 'Ibu Menyusui', 'Balita']
                };

                if (groupType === '-') {
                    errors.push('TIPE KELOMPOK Kosong');
                } else if (!allowedGroup.includes(groupType.toString().toLowerCase().trim())) {
                    errors.push('TIPE KELOMPOK Tidak Valid');
                } else {
                    // Validasi Kategori jika tipe kelompok valid
                    const currentGroupTypeLower = groupType.toString().toLowerCase().trim();
                    if (category === '-') {
                        errors.push('KATEGORI Kosong');
                    } else if (validCategories[currentGroupTypeLower]) {
                        // Case-insensitive check for category
                        const isValidCategory = validCategories[currentGroupTypeLower].some(
                            validCat => validCat.toLowerCase() === category.toString().toLowerCase().trim()
                        );
                        if (!isValidCategory) {
                             errors.push('KATEGORI Tidak Valid untuk Tipe ' + groupType);
                        }
                    }
                }

                // Validasi Internal (Wajib di semua mode)
                if (rawCode && seenCodes.has(rawCode)) errors.push('KODE PM ganda di file ini');
                ...
                const parseStatus = '<span class="text-xs font-medium px-1.5 py-0.5 rounded border capitalize bg-emerald-100 text-emerald-600 border-emerald-200">Aktif</span>';

                processedRowsHTML.push(`
                <tr class="${isRowError ? 'bg-rose-50/50' : 'hover:bg-slate-50'} transition-all text-[12px]">
                    <td class="p-3 border-b border-slate-100 text-slate-600">${groupType}</td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (finalForm) {
                finalForm.addEventListener('submit', function(e) {
                    e.preventDefault(); // Stop form agar tidak langsung kirim API biasa

                    // Langsung eksekusi script AJAX
        ...
        function executeAjaxImport(form) {
            ...
            // SEKARANG kita bisa melakukan disable tombol import_mode, KARENA formData SUDAH diambil. (Disabled input tidak terbaca FormData)
            ...
            .then(res => {
                ...
                if (res.status === 200) {
                    ...
                    if (res.body.errorDetails && res.body.errorDetails.length > 0) {
                        ...
                        bar.classList.replace('bg-emerald-500', 'bg-emerald-500'); // same class, no jump
