<div id="sppgEditModalContainer" x-show="showEditModal" class="fixed inset-0 z-[99] flex items-center justify-center p-4 text-left" x-cloak>
    <style>
        .input-disabled {
            ...
            <div class="p-8 max-h-[75vh] overflow-y-auto space-y-10 custom-scrollbar" id="editModalScrollContainer">
                ...
                <div class="pt-10 border-t border-gray-100">
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <input type="text" name="latitude_gps" id="e_lat" x-model="selectedUnit.latitude_gps" required readonly class="w-full mt-1 px-4 py-2.5 bg-slate-100 border-none rounded-lg text-sm text-slate-500" placeholder="Latitude Otomatis">
                        </div>
                        <div>
                            <label class="text-[11px] font-bold text-gray-400 uppercase">Longitude <span class="text-rose-500">*</span></label>
                            <input type="text" name="longitude_gps" id="e_lng" x-model="selectedUnit.longitude_gps" required readonly class="w-full mt-1 px-4 py-2.5 bg-slate-100 border-none rounded-lg text-sm text-slate-500" placeholder="Longitude Otomatis">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
                        ...
                        <div class="flex gap-2 relative">
                            ...
                            <button type="button"
                                ...
                                :disabled="stagedSuppliers.length === 0 || isLinkingSupplier"
                                ...
                                <span x-show="!isLinkingSupplier">Tautkan <span x-show="stagedSuppliers.length > 0" x-text="'(' + stagedSuppliers.length + ')'"></span></span>
                                <span x-show="isLinkingSupplier">...</span>
                            </button>
                        </div>
                ...
                <div class="pt-10 border-t border-gray-100" 
                    x-data="{ 
                        ...
                        resetCertForm() {
                            ...
                            if (document.getElementById('edit_cert_file')) document.getElementById('edit_cert_file').value = '';
                        },
                        formatDate(dateStr) {
                            if (!dateStr) return '-';
                        ...
                        isTypeSelected(type) {
                            return selectedUnit.certifications.some(c => 
                                c.name_certification === type && (!this.certEditing || c.id_certification !== this.certEditing.id_certification)
                            );
                        },
                        async saveCert() {
                            this.certErrors = {};
                            const formData = new FormData();
                            // Explicitly append fields to avoid sending irrelevant data from certForm
                            const fields = ['name_certification', 'certification_number', 'issued_by', 'issued_date', 'start_date', 'expiry_date'];
                            fields.forEach(key => {
                                formData.append(key, this.certForm[key] || '');
                            });
                            
                            formData.append('status', this.certForm.status ? 1 : 0);
                            formData.append('id_sppg_unit', selectedUnit.id_sppg_unit);

                            const fileInput = document.getElementById('edit_cert_file');
                            if (fileInput.files[0]) {
                                formData.append('file_certification', fileInput.files[0]);
                            }

                            const url = this.certEditing 
                                ? `/admin/manage-certification/${this.certEditing.id_certification}/update`
                                : `/admin/manage-certification/store`;
                            
                            if (this.certEditing) formData.append('_method', 'PATCH');
                            ...
                            try {
                                ...
                                if (resp.ok) {
                                    ...
                                    this.resetCertForm();
                                    // Refresh list
                                    const respList = await fetch(`/admin/manage-certification/by-unit/${selectedUnit.id_sppg_unit}`);
                                    selectedUnit.certifications = await respList.json();
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Berhasil',
                                        text: data.message,
                                    ...
                                    const data = await resp.json();
                                    this.certErrors = data.errors || {};
                                    // Change any specific error to the generic message if needed
                                    for (let key in this.certErrors) {
                                        this.certErrors[key] = ['* Wajib diisi'];
                                    }
                                } else {
                                    const data = await resp.json();
                        ...
                        async deleteCert(id) {
                            ...
                            if (result.isConfirmed) {
                                try {
                                    const resp = await fetch(`/admin/manage-certification/${id}`, {
                                        method: 'DELETE',
                                        headers: {
                                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                            'X-Requested-With': 'XMLHttpRequest',
                                    ...
                                    const data = await resp.json();
                                    if (data.success) {
                                        selectedUnit.certifications = selectedUnit.certifications.filter(c => c.id_certification != id);
                        ...
                        async toggleCertStatus(cert) {
                            try {
                                const resp = await fetch(`/admin/manage-certification/${cert.id_certification}/toggle-status`, {
                                    method: 'POST',
                                    headers: {
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                        'X-Requested-With': 'XMLHttpRequest',
                    ...
                    @init-edit-sppg.window="
                        setTimeout(async () => {
                            try {
                                const resp = await fetch(`/admin/manage-certification/by-unit/${$event.detail.id_sppg_unit}`);
                                selectedUnit.certifications = await resp.json();
                ...
                >
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        ...
                        <button type="button" @click="showCertForm = !showCertForm; certEditing = null; resetCertForm()" class="px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-emerald-100 transition-all flex items-center">
                            ...
                            <span x-text="showCertForm ? 'Batal' : 'Tambah Sertifikasi'"></span>
                        </button>
                    </div>
                    ...
                    <div x-show="showCertForm" x-collapse class="mb-8 p-6 bg-slate-50 rounded-xl border border-slate-200">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <template x-if="certEditing && certEditing.file_certification">
                                    <div class="flex items-center gap-2 mt-2 p-2 bg-indigo-50/50 rounded-lg border border-indigo-100/50">
                                        ...
                                        <div class="flex flex-col">
                                            <span class="text-[10px] text-indigo-700 font-medium">File Terdaftar:</span>
                                            <a :href="'/storage/' + certEditing.file_certification.replace('public/', '')" target="_blank" class="text-[10px] text-indigo-600 hover:text-indigo-800 underline font-bold">Lihat File Sekarang</a>
                                        </div>
                                    </div>
                                </template>
                                <p class="text-[10px] text-slate-400 mt-1 italic" x-show="certEditing">Kosongkan jika tidak ingin mengubah file</p>
                            </div>
                            <div class="flex items-center gap-4">
                                <button type="button" @click="saveCert()" class="mt-6 px-8 py-2.5 bg-indigo-600 text-white rounded-lg text-xs font-bold uppercase transition-all hover:bg-indigo-700 shadow-md active:scale-95">
                                    <span x-text="certEditing ? 'Simpan Perubahan' : 'Tambah Sertifikasi'"></span>
                                </button>
                            ...
                            <table class="w-full text-left text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    <template x-if="selectedUnit.certifications && selectedUnit.certifications.length > 0">
                                        <template x-for="c in selectedUnit.certifications" :key="c.id_certification">
                                            <tr class="hover:bg-indigo-50/30 transition-colors group">
                                                ...
                                                <td class="px-5 py-4 text-center">
                                                    <button type="button" @click="toggleCertStatus(c)" class="relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
                                                ...
                                                <td class="px-5 py-4 text-center">
                                                    <div class="flex justify-center items-center gap-1">
                                                        <template x-if="c.file_certification">
                                                            <a :href="'/storage/' + c.file_certification.replace('public/', '')" target="_blank" class="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg transition-all" title="Lihat PDF">
                                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                                                </svg>
                                                            </a>
                                                        </template>
                                                         <button type="button" @click="showCertForm = true; certEditing = c; certForm = {...c, status: !!c.status, issued_date: c.issued_date ? c.issued_date.split('T')[0] : '', start_date: c.start_date ? c.start_date.split('T')[0] : '', expiry_date: c.expiry_date ? c.expiry_date.split('T')[0] : ''}" 
                                                            class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-all" title="Edit">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                            </svg>
                                                        </button>
                                                        <button type="button" @click="deleteCert(c.id_certification)"
                                                            class="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="Hapus">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <tr>
                                            <td colspan="5" class="px-5 py-12 text-center">
                                                <div class="flex flex-col items-center">
                                                    ...
                                                    <p class="text-[11px] font-bold uppercase tracking-widest text-slate-400 italic">Belum ada sertifikasi terdaftar</p>
                                                </div>
                                            </td>
                                        </tr>
                                    </template>
...
<script>
    ...
    window.addEventListener('init-edit-sppg', function(e) {
        ...
        const latVal = unit.latitude_gps || unit.latitude || '';
        const lngVal = unit.longitude_gps || unit.longitude || '';
        document.getElementById('e_lat').value = latVal;
        document.getElementById('e_lng').value = lngVal;

        // Force update map marker jika ada koordinat
        if (latVal && lngVal) {
            if (typeof initEditMapModal === 'function') initEditMapModal(true);
        }
    ...
    // 3. LOGIKA PETA EDIT
    // ================================================================
    function initEditMapModal(forceUpdate = false) {
        ...
        const lat = parseFloat(latInput.value) || -8.11277341;
        const lng = parseFloat(lngInput.value) || 115.08833130;

        // Jika sudah ada instance, update view jika dipaksa dan panggil invalidateSize
        if (window.editMapInstance) {
            if (forceUpdate) {
                window.editMapInstance.setView([lat, lng], 15);
                if (window.editMarkerInstance) {
                    window.editMarkerInstance.setLatLng([lat, lng]);
                }
            }
            
            // Panggil berulang untuk memastikan benar-benar ter-render setelah transisi selesai
            setTimeout(() => window.editMapInstance.invalidateSize(), 150);
            setTimeout(() => window.editMapInstance.invalidateSize(), 600);
            return;
        }

        window.editMapInstance = L.map('map-edit').setView([lat, lng], 15);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 19,
            attribution: '© OpenStreetMap'
        }).addTo(window.editMapInstance);

        window.editMarkerInstance = L.marker([lat, lng], {
            draggable: true
        }).addTo(window.editMapInstance);
        ...
        window.editMapInstance.on('click', function(e) {
            ...
            if (typeof clearEditFieldError === 'function') clearEditFieldError('e_lat');
        });

        window.editMarkerInstance.on('dragend', function(ev) {
