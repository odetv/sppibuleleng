    ...
    <div id="nav-desktop">
        <div id="nav-content" class="flex items-center justify-between p-4 lg:px-8">
            ...
            <div class="hidden lg:flex lg:flex-1 lg:justify-end">
                ...
                <div class="relative" @click.away="profileOpen = false">
                    <button @click.prevent="profileOpen = !profileOpen" class="flex items-center gap-4 focus:outline-none cursor-pointer">
                        <span class="hidden text-right lg:block">
                            <span class="block text-sm font-semibold text-darkblue leading-none">
                                {{ Auth::user()->person->name ?? Auth::user()->email }}
                            </span>
                        </span>
                        <div class="h-11 w-11 rounded-full border border-gray-200 p-0.5 overflow-hidden shadow-sm">
                    ...
                    <div x-show="profileOpen"
                        ...
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 text-left">
                            <p class="text-sm font-semibold text-slate-800 truncate">{{ Auth::user()->person->name ?? 'Profil Belum Dilengkapi' }}</p>
                ...
                @else
                <a href="{{ route('login') }}" class="text-sm/6 font-semibold bg-blue-500 hover:bg-gold text-white px-4 py-2 rounded-md transition-colors shadow-sm">Masuk</a>
                @endauth
            </div>
        </div>
    </div>
                            ...
                            <div class="py-6">
                                <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 shadow-sm">
                                    <div class="flex items-center gap-4">
                                        <div class="h-14 w-14 rounded-full border-2 border-white shadow-md overflow-hidden flex-shrink-0">
                                            @if(Auth::user()->person && Auth::user()->person->photo)
                                                <img src="{{ asset('storage/' . Auth::user()->person->photo) }}" class="h-full w-full object-cover">
                                            @else
                                                <div class="flex h-full w-full items-center justify-center bg-indigo-100 text-indigo-600 font-bold uppercase text-lg">
                                                    {{ substr(Auth::user()->person->name ?? Auth::user()->email, 0, 1) }}
                                                </div>
                                            @endif
                                        </div>
                                        <div class="overflow-hidden text-left">
                                            <p class="text-sm font-bold text-darkblue truncate">{{ Auth::user()->person->name ?? 'Profil Belum Dilengkapi' }}</p>
                                            <p class="text-[11px] text-gray-500 truncate leading-tight mt-1">{{ Auth::user()->email }}</p>
                                        </div>
                                    </div>
