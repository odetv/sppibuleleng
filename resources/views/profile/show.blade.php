                    ...
                    <div class="flex flex-col md:flex-row items-center space-y-6 md:space-y-0 md:space-x-12">
                        ...
                        <div class="flex-1 text-center md:text-left">
                            <div class="mb-6">
                                ...
                                <h3 class="text-xl sm:text-3xl font-bold text-gray-900 tracking-tight mb-2">
                                    {{ $fullName }}
                                </h3>
                                <div class="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-3">
                                    <span class="inline-flex items-center px-3 py-2.5 text-[10px] font-bold rounded-md bg-indigo-50 text-indigo-600 uppercase border border-indigo-100 tracking-wider">
                                        {{ Auth::user()->role->name_role ?? '-' }}
                                    </span>
                                    <span class="inline-flex items-center px-3 py-2.5 text-[10px] font-bold rounded-md {{ Auth::user()->status_user == 'active' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-yellow-50 text-yellow-600 border-yellow-100' }} uppercase border tracking-wider">
                                        {{ Auth::user()->status_user }}
                                    </span>
                                    <a href="{{ route('profile.edit') }}" class="inline-flex items-center px-5 py-2.5 bg-white border border-slate-200 rounded-lg font-bold text-[10px] text-slate-700 uppercase tracking-widest hover:bg-slate-50 transition">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-5 mt-8 pt-6 border-t border-gray-50 text-sm text-left">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <span class="font-semibold text-gray-700 tracking-wide">{{ Auth::user()->person->no_kk ?? '-' }}</span>
                                </div>
                                <div class="flex items-center space-x-4">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <span class="font-semibold text-gray-700 tracking-wide">{{ Auth::user()->person->npwp ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-8">
                    ...
                    <dl class="space-y-6 text-sm">
                        <div class="flex flex-col space-y-1.5 border-b border-gray-50 pb-4">
                            <dt class="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mb-1">Tempat, Tanggal Lahir</dt>
                            <dd class="font-semibold text-gray-800">{{ Auth::user()->person->place_birthday ?? '-' }}, {{ Auth::user()->person && Auth::user()->person->date_birthday ? \Carbon\Carbon::parse(Auth::user()->person->date_birthday)->translatedFormat('d F Y') : '-' }}</dd>
                        </div>
                        <div class="flex flex-col space-y-1.5 border-b border-gray-50 pb-4">
                            <dt class="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mb-1">Jenis Kelamin</dt>
                            <dd class="font-semibold text-gray-800">{{ Auth::user()->person && Auth::user()->person->gender == 'L' ? 'Laki-laki' : 'Perempuan' }}</dd>
                        </div>
                        <div class="flex flex-col space-y-1.5 border-b border-gray-50 pb-4">
                            <dt class="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mb-1">Usia</dt>
                            <dd class="font-semibold text-gray-800">{{ Auth::user()->person->age ?? '-' }} Tahun</dd>
                        </div>
                        <div class="flex flex-col space-y-1.5 border-b border-gray-50 pb-4">
                            <dt class="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mb-1">Agama</dt>
                            <dd class="font-semibold text-gray-800">{{ Auth::user()->person->religion ?? '-' }}</dd>
                        </div>
                        <div class="flex flex-col space-y-1.5 border-b border-gray-50 pb-4">
                            <dt class="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mb-1">Status Perkawinan</dt>
                ...
                <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-8">
                    ...
                    <dl class="space-y-5 text-sm">
                        ...
                        <div class="grid grid-cols-2 gap-8">
                            ...
                            <div class="flex flex-col space-y-1.5 border-b border-gray-50 pb-3">
                                <dt class="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Alamat Lengkap</dt>
                ...
                <div class="md:col-span-2 bg-white shadow-sm sm:rounded-xl border border-gray-100 p-8">
                    <div class="flex flex-wrap justify-between items-center mb-8 gap-4">
                        ...
                        <a href="https://www.google.com/maps/search/?api=1&query={{ Auth::user()->person->gps_coordinates }}"
                            target="_blank"
                            class="inline-flex items-center px-5 py-2 bg-indigo-600 border border-transparent rounded-lg font-bold text-[10px] text-white uppercase tracking-widest hover:bg-indigo-700 transition shadow-md">
                    ...
                    <div id="map-preview" class="h-80 rounded-2xl border-4 border-white shadow-lg ring-1 ring-gray-100 z-0 mb-6"></div>

                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                        <div class="flex items-center gap-3">
                            <div class="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse ring-4 ring-emerald-100"></div>
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Koordinat Terpilih</span>
                        </div>
                        <span class="font-mono text-indigo-600 font-bold text-sm px-6 py-2 bg-white rounded-xl shadow-sm border border-indigo-50 tracking-tighter">
